...
def plotter(x,avg_array,std_array,title,save_fig,IMG_FOLDER,NUMBER_OF_CARS,TIME_HORIZON):
    avg_array = np.array(avg_array)
    std_array = np.array(std_array)
    plt.style.use('ggplot')
    fig, ax = plt.subplots(figsize=(10,5))
    ax.plot(x,avg_array,color='red',label='average '+str(title).lower().replace(' ','_'))
