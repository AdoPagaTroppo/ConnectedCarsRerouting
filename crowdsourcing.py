...
class Crowdsourcing():
    def __init__(self, agent, numalgs):
        ...
        self.edgeNum = self.behaviors.shape[1]
        self.targetNum = self.behaviors.shape[0]
    ...
    def receding_horizon_DM(self, state, tHor, stateSpace, r, online = False, behaviours = None):
        ...
        self.PMF_target = self.behaviors[self.targetIndex]
        for i in range(dim):
            # print("index "+str(i))
            # if not online:
        ...
        for i in range(self.numalgs):
            for j in range(dim):
                ...
                # if not online:
                    sources[i,j] = self.behaviors[i+self.targetIndex,stateSpace[j]]
                # else:
